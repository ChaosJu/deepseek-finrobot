...
## Contrarian Trading Bot

This bot implements a contrarian trading strategy based on market psychology, aiming to 'buy fear and sell greed'. It uses a multi-agent system built with `pyautogen` and leverages the DeepSeek LLM for advanced analysis.

### Prerequisites

1.  **Python Environment:** Python 3.9+ is recommended.
2.  **Dependencies:** Install all necessary dependencies. If you have followed the main project installation (`bash install.sh` or `pip install -r requirements.txt`), most should be covered. Key specific dependencies for the agents include `pyautogen`.
3.  **API Keys:**
    *   **DeepSeek API Key:** You must have a valid DeepSeek API key. Configure it in `config_api_keys.json` at the root of the project, like so:
        ```json
        {
          "DEEPSEEK_API_KEY": "your-deepseek-api-key",
          "OPENAI_API_KEY": "placeholder" // If not using OpenAI directly for this
        }
        ```
    *   **(Optional) Data Source API Keys:** If you extend the bot to use other data sources (e.g., Alpha Vantage for international markets), those API keys will also need to be configured as per their requirements. `akshare` (used for Chinese market data) generally doesn't require an API key for public data.

### Configuration

1.  **API Keys:** Ensure `config_api_keys.json` is correctly set up as described above.
2.  **Strategy Configuration File:**
    *   The contrarian bot uses a YAML or JSON configuration file to define its strategy parameters. Create a file (e.g., `contrarian_strategy_config.yaml`).
    *   **Example `contrarian_strategy_config.yaml`:**
        ```yaml
        ...
        contrarian_logic_params:
          ...
          VOLUME_SPIKE_FACTOR: 1.5 # e.g., current volume must be 1.5x average
          RSI_OVERSOLD_THRESHOLD: 30
          RSI_OVERBOUGHT_THRESHOLD: 70
          MIN_RUNUP_PERCENTAGE_FOR_SELL_SIGNAL: 10 # Price must be up by 10% recently for good news to be a strong sell

        # LLM configurations for agents (example, use models available to your DeepSeek API key)
        llm_configs:
          ceo_agent:
          ...
          news_sentiment_agent:
            model: "deepseek-chat"
            temperature: 0.7
          # market_data_agent might not use an LLM directly for its core tasks

        # (Optional) Other global settings for the bot
        # portfolio_max_allocation_per_asset: 0.10 # Example: max 10% of portfolio in one asset
        ```
    *   **Key Parameters in `contrarian_logic_params`:**
        *   `THRESHOLD_EXTREME_NEGATIVE`/`POSITIVE`: Sentiment score thresholds to trigger contrarian logic.
        *   `VOLUME_SPIKE_FACTOR`: Multiplier for average volume to confirm panic/euphoria.
        *   `RSI_OVERSOLD`/`OVERBOUGHT_THRESHOLD`: Standard RSI levels for overbought/oversold conditions.
        *   `MIN_RUNUP_PERCENTAGE_FOR_SELL_SIGNAL`: How much an asset's price should have risen recently for positive news to be considered a strong sell signal (to catch "sell the news" scenarios after a rally).

### Running the Bot

The bot is launched via the main `cli.py` script:
...
*   **`--assets <ASSET_LIST>`:** (Required) Comma-separated list of asset IDs (stock tickers) to analyze. Example: `000001.SZ,600519.SH`
*   **`--config-file <PATH_TO_STRATEGY_CONFIG>`:** (Required) Path to your strategy configuration YAML/JSON file (e.g., `contrarian_strategy_config.yaml`).
*   **`--output-log <PATH_TO_LOG_FILE>`:** (Optional) Path to the file where trading decisions and rationale will be logged. Defaults to `contrarian_trades.log` in the current directory. Example: `logs/my_contrarian_trades.md`

**Example Command:**

```bash
python -m deepseek_finrobot.cli contrarian_trader --assets 000001.SZ,600519.SH --config-file configs/contrarian_strategy_config.yaml --output-log logs/trades.md
```

### Interpreting the Output Log (`--output-log`)

The output log file (e.g., `contrarian_trades.log`) will contain timestamped entries for each decision made by the bot. Each entry will typically include:

*   **Timestamp:** When the decision was made.
*   **Asset ID:** The asset being analyzed.
*   **News Sentiment Summary:** Key sentiment findings.
*   **Market Data Summary:** Relevant price, volume, and indicator data.
*   **Contrarian Signal:** The signal generated by the `ContrarianSignalGenerator` (e.g., BUY, SELL, STRONG_BUY) with its confidence score and justification.
*   **CEO's Final Decision:** The final action taken by the `ChiefExecutiveOfficerAgent` (e.g., "Action: BUY, Quantity: 100 shares (simulated), Rationale: Strong contrarian buy signal due to panic selling...").

This log serves as the detailed record of the bot's activity and reasoning, crucial for review and strategy refinement.
